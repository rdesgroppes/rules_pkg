...
def test_referencing_remote_file(name):
    tests = [
        ...
        struct(name = "{}_strip_prefix".format(name), strip_prefix = strip_prefix.from_root("tests")),
        # ... which should now match the default behavior, where the external package prefix is automatically stripped
        struct(name = "{}_default".format(name), strip_prefix = None),
    ]
